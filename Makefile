...
DIAGRAMS_PATH := docs
PLANTUML_JAR = ${DIAGRAMS_PATH}/plantuml.jar
DIAGRAMS_SRC := $(wildcard docs/*.puml)
...
docs: $(PLANTUML_JAR) $(DIAGRAMS_PNG) # Default target first; build PNGs, probably what we want most of the time.

$(PLANTUML_JAR): # If the JAR file isn't already present, download it.
	curl -sSfL $(PLANTUML_JAR_URL) -o $(PLANTUML_JAR)

$(DIAGRAMS_PATH)/%.png: $(DIAGRAMS_PATH)/%.puml # Each PNG output depends on its corresponding .puml file.
